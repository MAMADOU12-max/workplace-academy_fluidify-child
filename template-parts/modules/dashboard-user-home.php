...
$every_course_view=array();
if(!empty($views)){
    $view = get_field('view', $views[0]->ID);
foreach($course_types as $course_type){
    $count=0;
    foreach($view as $view_element)
    {
        $course = $view_element['view_course'];
        if(get_field('course_type', $course->ID) == $course_type)
            $count++;      
    }
    array_push($every_course_view,$count);
}
}
$count=0;
 //var_dump($every_course_view);
 $final_course_type=array();
 foreach ($every_course_view as $key => $value) {
    if ($count>3)
        break;
        $max=max($every_course_view);
    if ($max==$value && $max!=0) {
        var_dump($max);
        $count++;
        array_push($final_course_type,$course_types[$key]);
        unset ($every_course_view[$key]);
    }
 } 
 var_dump($every_course_view);
    var_dump($final_course_type);
// 
// $count_max=0;
...
//         break;
//     $max= max($every_course_view);
//     if($every_course_view[$key]==$max)
//     {
//         $count_max++;
//         array_push($final_course_type,$value);
//         unset($course_types[$key]);
//     }
    
// }
// var_dump(max($every_course_view));

?>
<?php
    if (in_array('Opleidingen', $final_course_type)) {
?>
...
?>
<?php
    if (in_array('E-learning', $final_course_type)) {
?>
<div class="block-treaning">
...
?>
<?php
    if (in_array('Workshop', $final_course_type)) {
?>
<div class="block-treaning">
...
?>
<?php
    if (in_array('Masterclasses', $final_course_type)) {
?>
<div class="block-treaning">
...
?>
<?php
    if (in_array('Event', $final_course_type)) {
?>
...
?>
<?php
    if (in_array('Video', $final_course_type)) {
?>
<div class="block-treaning">
...
?>
<?php
    if (in_array('Cursus', $final_course_type)) {
?>
<div class="block-treaning">
